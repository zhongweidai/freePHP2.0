 ...
 * 日志记录的级别有以下几种：
 * <ul>
 * <li>FreeLogger::LEVEL_INFO = 1: 信息记录，只记录需要记录的信息。</li>
 * <li>FreeLogger::LEVEL_TRACE = 2: 信息记录，同时记录来自trace的信息。</li>
 * <li>FreeLogger::LEVEL_DEBUG = 3: 信息记录，同时记录来自trace的信息。</li>
 * <li>FreeLogger::LEVEL_ERROR = 4: 记录错误信息，不包含trace信息。</li>
 * <li>FreeLogger::LEVEL_PROFILE = 5: 分析信息记录，包含详细的时间及内存使用情况等</li>
 * </ul>
...
class FreeLogger{
	 ...
	 * @param string $type 日志的类型,默认为free.system
	 * @param boolean $flush 是否将日志输出到文件,为true的时候将写入文件,默认为false
	 * @return void
	 */
	public function info($msg, $type = 'free.system', $flush = false) {
	 ...
	 * @return void
	 */
	public function trace($msg, $type = 'free.system', $flush = false) {
	 ...
	 * @return void
	 */
	public function debug($msg, $type = 'free.system', $flush = false) {
	 ...
	 * 
	 * @param string $msg 日志信息
	 * @param string $type 日志的类型,默认为free.core
	 * @param boolean $flush 是否将日志输出到文件,为true的时候将写入文件,默认为false
	 * @return void
	 */
	public function error($msg, $type = 'free.core', $flush = false) {
	 ...
	 * @return void
	 */
	public function profileBegin($msg, $type = 'free.core', $flush = false) 
    {
	 ...
	 * @return void
	 */
	public function profileEnd($msg, $type = 'free.core', $flush = false) 
    {
	 ...
	 * @return void
	 */
	public function log($msg, $level = self::LEVEL_INFO, $type = 'free.system', $flush = false) 
    {
		if (!$this->_logDir) 
        {
            return;    
	...
	public function flush() 
    {
		...
		free::import('free:utility.FreeFile');
		...
		if ($this->_writeType & 1) 
        {
			foreach ($_logLevels as $key => $value) 
            {
				...
				FreeFile::write($fileName, join("", $value), 'a');
			}
		}
        ...
        {
			FreeFile::write($fileName, join("", $_l), 'a');
		}
		$this->_logs = array();
		$this->_logCount = 0;
		return true;
	...
	private function _getTrace() {
		...
		foreach ($traces as $traceKey => $trace) {
			...
			if ($function == 'freeBase::log') continue;
	...
	public function setLogDir($logDir) {
		if (!is_dir($logDir)) $logDir = free::getRealDir($logDir);

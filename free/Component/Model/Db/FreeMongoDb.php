 ...
 public function __construct($container) {
	...
	public function insert($data, $table, $return_insertId = false, $relpace = false)
	{
		...
		try {
			...
			//生成自增ID
			$data['ID'] = $this->getMongoAutoId($table);
/*			$this->registerComponent(array('sqs_redis'=>'free/libs/cache/FreeRedisCache'));
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function query($command,$key='') {
...
**/
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function parseThinkWhere($key,$val) {
        $query   = array();
        switch($key) {
     ...
     * @version 2013-11-25
     */
	public function getMongoAutoId ($collection = 'index_auto')
	{
	    $update = array('$inc' => array('id' => 1));
	    $query = array('table' => $collection);
	    $command = array('findandmodify' => '_increase', 'update' => $update,'query' => $query, 'new' => true, 'upsert' => true);
	    $rs = $this->_db->command($command);
	    return $rs['value']['id'];
	}

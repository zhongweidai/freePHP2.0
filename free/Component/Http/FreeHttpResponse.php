 ...
 * @subpackage response
 */
class FreeHttpResponse implements IFreeResponse {
	/**
	 * Status code (100)
	 *
	...
	*/
	...
	/* (non-PHPdoc)
	 * @see IFreeResponse::sendResponse()
	 */
	...
	public function isSendedHeader($throw = false) {
		...
		if ($throw && $sended) throw new FreeException(
	...
	*/
	...
	/* (non-PHPdoc)
	 * @see IFreeResponse::setData()
	 */
	public function setData($data, $key = '', $merge = false) {
		if ($key) {
			if ($merge && !empty($this->_data[$key])) {
				$this->_data[$key] = FreeUtility::mergeArray((array) $this->_data[$key], (array) $data);
			} else
				$this->_data[$key] = $data;
		} else {

...
class FreeRedisCache extends AbstractFreeCache{
    ...
    public function __construct($container)
	{
		...
		if (!is_object('redis')) 
		{
			$config = $this->_container->loadConfig('cache','redis');
			$this->redis = new Redis();
			try 
			{
				$this->_conn = $this->redis->connect($config[0]['hostname'], $config[0]['port'], $config[0]['timeout']);
			} 
			catch (Exception $e) 
			{
	 ...
	 * @param	string	$module		所属模块
	 */
	public function get($name, $module='common')
	{
		if(is_array($name))
		{

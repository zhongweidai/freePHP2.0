...
class FreeFileCache extends AbstractFreeCache{
	...
	public function __construct($container) {
        ...
        $setting = $this->_container->loadConfig('cache','file');
	...
	public function set($name, $data, $module='common',$expire=1800) {
		if(!is_string($name))
		{
			throw new FreeException('the name of FileCache must be string.','002');
		}
		if(empty($module)) 
		{
			throw new FreeException(' 3th parameter ','110');
		}
		$filepath = FREE_PATH . $this->_setting['file_path'] . DIRECTORY_SEPARATOR . $module . DIRECTORY_SEPARATOR;
	    ...
	    //是否开启互斥锁
		if( $this->_container->loadConfig('system', 'lock_ex')) {
	...
	public function cacheinfo($name, $module='commons') {
		if(empty($module)) 
		{

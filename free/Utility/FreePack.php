...
class FreePack {
	 ...
	 * @return boolean
	 */
	public function packFromFileList($fileList, $dst, $packMethod = FreePack::STRIP_PHP, $compress = true) {
		...
		return FreeFile::write($dst, '<?php' . $replace . $content . '?>');
	}
	 ...
	 * @return array:
	 */
	public function readContentFromFileList(array $fileList, $packMethod = FreePack::STRIP_PHP, &$content = array()) {
	...
	private function isValidatePackMethod($packMethod) {
		return method_exists($this, $packMethod) && in_array($packMethod, 
			array(FreePack::STRIP_PHP, FreePack::STRIP_SELF, FreePack::STRIP_TOKEN));
	}
}
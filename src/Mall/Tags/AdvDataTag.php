...
class AdvDataTag extends ServiceBase
{
    private $advModel;
    ...
    function advShow($ap_id, $type = 'js')
    {
        ...
        $time    = TIMESTAMP;
        //加载广告位缓存文件
        $ap_info = $this->getService('Mall:System.CacheService')
            ->call('Mall:Adv.AdvService::getAdsByPosition',array('ap_id'=>$ap_id));
        ...
        if(isset($ap_class) && $ap_class == '2')
        {
            ...
            if(isset($is_use) && $is_use == '1')
            {
                ...
                if(empty($adv_list)){
                    $pic = $this->advModel->getAttach($default_content);
                    ...
                    foreach ($adv_list as $v){
                        ...
                        $pic = $this->advModel->getAttach($pic);
            ...
            switch (isset($ap_display) ? $ap_display : 0){
                case '1'://多广告随机展示
                    /**
                        ...
                        if(empty($adv_list)){
                            switch ($ap_class){
                                case '0':
                                    ...
                                    $content .= $this->advModel->getAttach($default_content);
                                    $content .= "' alt=''/>";
                                    $content .= "</a>\");";
                                    break;
                                case '1':
                            ...
                            if($ap_class == '0' && $is_use == '1'){
                                ...
                                $content .= "<img style='width:{$width}px;height:{$height}px' border='0' src='";
                                $content .= $this->advModel->getAttach($pic);
                            ...
                            if($ap_class == '3' && $is_use == '1'){
                                ...
                                $content .= $this->advModel->getAttach($flash);
                                ...
                                $content .= ServiceUtil::assets('data/resource/js/expressInstall.swf') . "'/><!-- 下一个对象标签用于非 IE 浏览器。所以使用 IECC 将其从 IE 隐藏。 --><!--[if !IE]>--><object type='application/x-shockwave-flash' data='";
                                $content .= $this->advModel->getAttach($flash);

...
//         echo $this->_xs_search->getQuery();
    ...
    private function _createXS($pagesize,$appname='') {
        $curpage = $this->getContainer()->getComponent('request')->getGet('curpage');
        ...
        $search = new Xs();  
        $this->_xs_search = $search->getSearch();
        $this->_xs_search->setCharset($this->getContainer()->loadConfig('system','charset'));
    ...
    private function _searchXS(){
        try {
            ...
            $goods_class = $this->getService('Mall:System.CacheService')->call('Mall:Goods.GoodsClassService::getGoodsClass');
...
//                 }
            ...
            throw new Free\Libs\FreeException($e->getMessage());
     ...
     */
    public function getAttrList($get){
        //得到分类ID
        $cate_id = intval($get['cate_id']) ? intval($get['cate_id']) : 0 ;
        //得到品牌id
        $brand_id = $get['b_id'];
        //得到属性id
        $attr_id = $get['a_id'];
        ...
        //先查找$hash_key缓存
        $data = $this->getCacheCompenont()->get($hash_key,'search_p');
        if (!$data) {
            ...
            // 获取当前的分类内容
            $class_array = $this->getService('Mall:System.CacheService')->call('Mall:Goods.GoodsClassService::getGoodsClass');
            ...
            if (!empty($data) && count($data['gcid_array']) == 1) {
                ...
                if (is_array($param['attr_id'])) {
                    ...
                    foreach ($param['attr_id'] as $val) {
                        ...
                        $goodsattrindex_list = $this->getModel('Mall:GoodeAttrIndex')->getGoodsAttrIndexList($where, 'goods_id');
                ...
                //品牌列表
                $typebrand_list = $this->getService('Mall:Goods.GoodsTypeService')->getTypeBrandList(array('type_id' => $data['type_id']), 'brand_id');
                if (!empty($typebrand_list)) {
                    $brandid_array = array();
                    foreach ($typebrand_list as $val) {
                        $brandid_array[] = $val['brand_id'];
                    }
                    $brand_array = $this->getService('Mall:Goods.GoodsTypeService')->getBrandPassList(array('brand_id' => array('in', $brandid_array)), 'brand_id,brand_name');
                ...
                //属性列表
                $attribute_list = $this->getService('Mall:Goods.AttributeService')->getAttributeShowList(array('type_id' => $data['type_id']), 'attr_id,attr_name');
                $attributevalue_list = $this->getService('Mall:Goods.AttributeService')->getAttributeValueList(array('type_id' => $data['type_id']), 'attr_value_id,attr_value_name,attr_id');
                $attributevalue_list = FreeArray::arrayUnderReset($attributevalue_list, 'attr_id', 2);
                $attr_array = array();
                if (!empty($attribute_list)) {
                    foreach ($attribute_list as $val) {
                        $attr_array[$val['attr_id']]['name'] = $val['attr_name'];
                        $tpl_array = FreeArray::arrayUnderReset($attributevalue_list[$val['attr_id']], 'attr_value_id');
                ...
                //缓存规格组合结果
                $data = $this->getCacheCompenont()->set($hash_key,$data,'search_p');
            }
        }
        return $data;
    }
   ...
   public function getTagCategory($keyword = '') {
        if ($keyword != '') {
            ...
            $tag_list = $tag =$this->getService('Mall:System.CacheService')->call('Mall:Goods.GoodsClassService::getGoodsClassTag');
     ...
     */
     public function getLeftCategory($param, $sign = 0) {
        ...
        $tpl_data = array();
        $gc_list = $this->getService('Mall:Goods.GoodsClassService')->getAllCategory();
        foreach ($data as $value) {
            ...
            if (!empty($gc_list[$value[0]])){   // 一级
                ...
                if (!empty($gc_list[$value[0]]['class2'][isset($value[1]) ? $value[1] : 0])) { // 二级
